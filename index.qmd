...
::: {.challenge-grid}
::: {.challenge-card}
**Reactive Gap Following — Short Run**
...
::: {.challenge-card}
**Reactive Gap Following — Long Run**
...
::: {.challenge-card}
**A\* + Stanley Controller — Short Run**

::: {.video-container}
{{< video https://www.youtube.com/watch?v=drcnit15shc >}}
:::

Short simulation run of the map-based pipeline: A* plans a path from the current pose to a goal set in RViz, and the Stanley controller tracks it with combined heading and cross-track correction.
:::
:::
...
### A\* + Stanley Controller — Full Simulation Lap

::: {.content-text}
The following five plots are recorded from a complete simulation lap of the A* + Stanley pipeline (~160 seconds, ~2.5 laps).
:::
...
**Trajectory Trace (color = speed)** — The Stanley controller follows the A*-planned path around the full track. The car holds high speed (yellow/green, 5–6 m/s) on all straights, with clean speed reductions (blue, 2–3 m/s) at each corner. The single consistent trace line confirms the controller tracks the reference path with no visible deviation or oscillation throughout the lap.
:::

::: {.challenge-card}
![](<steering A.png>)

**Steering vs Time** — Commanded steering (blue) and safety-output steering (orange) overlap almost perfectly for the entire 160-second run, confirming the safety layer rarely needs to override the Stanley output. Steering stays well within ±0.1 rad after the large initial correction at startup (−0.4 rad when the controller first locks onto the path), demonstrating the smooth, low-amplitude tracking behavior of the Stanley equation.
:::

::: {.challenge-card}
![](<Acceleration A.png>)

**Acceleration vs Time** — Sharp spikes up to ±10 m/s² occur at predictable, regularly-spaced intervals corresponding to corner entry and exit events, with smooth near-zero periods in between on the straights. The periodic structure reflects the planned path: the car knows where corners are and brakes for them consistently each lap, in contrast to the reactive controller's high-frequency noise throughout.
:::

::: {.challenge-card}
![](<TTC A.png>)

**TTC vs Time** — Time-to-collision plotted against two thresholds: TTC slow (orange dashed, 0.8 s) and TTC brake (red dashed, 0.3 s). The car operates in the 0.3–0.8 s band throughout, approaching the brake threshold at the tightest corners near t = 95 s and t = 150 s but never triggering an emergency stop. The consistent operating range confirms the curvature-based velocity profile keeps the car at an appropriate speed for each section of track.
:::
:::

::: {.flowchart-wrap}
![](<shield speed A.png>){width="92%"}
:::

::: {.content-text}
**Safety Shield Speed Reduction vs Time** — The safety shield fires regularly across all 160 seconds, reducing the commanded speed by up to ~6 m/s at each intervention. The consistent ~5–10 second spacing between spikes maps directly to the corner frequency of the track layout, showing the shield responds to genuine geometric hazards rather than spurious noise. Between corners the reduction is zero, meaning the shield is not fighting the planner on the straights.
:::
